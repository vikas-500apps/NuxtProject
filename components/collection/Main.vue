    ...
    <div v-else>
        ...
        <div class="grid grid-rows-3 grid-flow-col grid-cols-4 border w-[80vw] mx-auto my-5 rounded-lg pr-[4px]">
            <div class="row-span-3 bg-gray-50 border-r p-5 rounded-l-lg h-[calc(103vh-150px)] overflow-auto">
                ...
                <CollectionList :templates="templates" @select="select" @delete="deleteItem" />
            </div>
            <!-- End of LHS section -->
            ...
            <CollectionEdit
                ...
                @edit="edit"
            />
            <p v-show="!templates.length && !addTemplate" class="mt-[150px] justify-center text-xl text-b ml-[400px]">
                {{ heading }}
            </p>
            <!-- End of RHS section -->
        </div>
...
const { pending, data: templates } = await useAuthLazyFetch(`${props.url}?offset=0&limit=100&sort_column=id&sort_direction=desc`, {
    method: 'GET',
    headers: authHeader,
})
...
const deleteItem = (templateUid: string) => {
    ...
    if (response) {
        const index = templates.value.findIndex((template: any) => template.uid === templateUid)
        if (index !== -1) {
